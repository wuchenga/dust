...
### 编辑docker-compose.yml文件添加: - CUSTOM_SHELL_FILE=https://raw.githubusercontent.com/wuchenga/dust/dust/shell_script_mod.sh
#### 容器完全启动后执行 docker exec -it jd_scripts /bin/sh -c 'crontab -l' 查看目前修行的经书

function wuchenga(){
    # https://github.com/wuchenga/dust
    rm -rf /wuchenga /scripts/wuchenga_*
    git clone https://github.com/wuchenga/dust.git /wuchenga
    # 拷贝脚本
    for jsname in $(find /wuchenga -name "*.js" | grep -vE "\/backup\/"); do cp ${jsname} /scripts/wuchenga_${jsname##*/}; done
    # 匹配js脚本中的cron设置定时任务
    for jsname in $(find /wuchenga -name "*.js" | grep -vE "\/backup\/"); do
        jsnamecron="$(cat $jsname | grep -oE "/?/?cron \".*\"" | cut -d\" -f2)"
        test -z "$jsnamecron" || echo "$jsnamecron node /scripts/wuchenga_${jsname##*/} >> /scripts/logs/wuchenga_${jsname##*/}.log 2>&1" >> /scripts/docker/merged_list_file.sh
    done
}
